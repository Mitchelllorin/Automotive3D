...
for ($i = 0; $i -lt 80; $i++) {
    if (Test-Path $log) {
        # Vite wraps the port in ANSI colour codes (localhost:<ESC>[1m5173<ESC>[22m),
        # so strip ESC sequences before matching or the digits never follow "localhost:".
        $raw = (Get-Content $log -Raw -ErrorAction SilentlyContinue) -replace "$([char]27)\[[0-9;]*m", ''
        if ($raw -match 'localhost:(\d+)') { $port = $matches[1]; break }
    }
